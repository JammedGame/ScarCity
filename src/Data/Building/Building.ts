...
class Building extends TBX.Tile
{
    ...
    public Init() : void
    {
        if(this._Unitar)
        {
            this.Collection = new TBX.ImageCollection(null, ["/Textures/Buildings/"+this._BID+"/icon.png"]);
            this.Index = 0;
            this.Active = false;
        }
        for(let i = 0; i < this._Structure.Size.X; i++)
        {
            for(let j = 0; j < this._Structure.Size.Y; j++)
            {
                if(this._Structure.Fields[j*this._Structure.Size.X + i] == 1)
                {
                    ...
                    ["/Textures/Buildings/"+this._BID+"/chunk_"+i+"_"+j+".png"]);
                    else Satelite = TBX.SceneObjectUtil.CreateTile(this._BID+"_"+i+"_"+j,
                    ["/Textures/Buildings/"+this._BID+"/unitar.png"]);
